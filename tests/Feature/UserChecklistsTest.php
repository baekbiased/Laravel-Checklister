...
class UserChecklistsTest extends TestCase
{
    ...
    public function test_can_load_checklist_page(){
        ...
        // Test that checklist is cloned to the user checklist
        $user_checklist = Checklist::where('checklist_id', $checklist->id)->where('user_id', auth()->id());
        $this->assertNotNull($user_checklist);

        // Test that checklist is shown on the page
    ...
    public function test_can_complete_task(){
        ...
        $user_task = Task::where('task_id', $task)
